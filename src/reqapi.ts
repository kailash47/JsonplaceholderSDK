...
export function ReturnObject<T>(status: boolean, message: string | any, code: string | any, data: T): ResponseObj<T> {
    return { status, message, code, data };
};

export async function buildRequestOptions<T>(config: ConfigParam, path: PathConfig<T>, datas?: T) {
    let options: any = {
    ...
    const params = Util.isObject(path.queryparams) ? querystring.stringify(path.queryparams) : undefined;
...
export function JsonRest<T>(config: ConfigParam, path: PathConfig<T>, data?: T | any): Promise<ResponseObj<T>> {
    return new Promise<ResponseObj<T>>(async (resolve, reject) => {
        ...
        const req = https.request(build, (res: IncomingMessage) => {
            ...
            res.on('end', () => {
                ...
                resolve(
                    ReturnObject<T>(
                        !((res && res.statusCode ? (res.statusCode < 200) : false) || (res && res.statusCode ? (res.statusCode >= 300) : false)),
                        res.statusMessage,
                        res.statusCode,
                        body
                    )
